...
//std::deque<float> faceWidth;
//std::deque<float> prevX;
//std::deque<float> prevY;
	...
	bool display(const std::shared_ptr<std::vector<op::Datum>>& datumsPtr)
	{
		...
		if (datumsPtr != nullptr && !datumsPtr->empty())
		{
			...
			for (auto person = 0; person < poseKeypoints.getSize(0); person++)
			{
				...
				auto x = poseKeypoints[{person, 0, 0}];
				auto y = poseKeypoints[{person, 0, 1}];
				const auto score = poseKeypoints[{person, 0, 2}];
				...
				if (score != 0)
				{
					...
					{
						faceWidth = std::abs(poseKeypoints[{person, 17, 0}] - poseKeypoints[{person, 0, 0}]);

					}
					else if (poseKeypoints[{person, 17, 2}] == 0 && poseKeypoints[{person, 18, 0}] != 0)
					{
						faceWidth = std::abs(poseKeypoints[{person, 0, 0}] - poseKeypoints[{person, 18, 0}]);
					}

					float px, py, wx = faceWidth, wy = faceWidth;

					px = poseKeypoints[{person, 0, 0}] - wx / 2 < 0 ? 0 : poseKeypoints[{person, 0, 0}] - wx / 2;
					py = poseKeypoints[{person, 0, 1}] - wy / 2 < 0 ? 0 : poseKeypoints[{person, 0, 1}] - wy / 2;

					px = px < outImage.cols - 1 - wx ? px : outImage.cols - 1 - wx;
					py = py < outImage.rows - 1 - wy ? py : outImage.rows - 1 - wy;
			...
			//float meanX = std::accumulate(prevX.begin(), prevX.end(), 0.0f) / prevX.size();
			//float meanY = std::accumulate(prevY.begin(), prevY.end(), 0.0f) / prevY.size();
...
int main(int argc, char *argv[])
{
	...
	window = glfwCreateWindow(640, 480, "optest", nullptr, nullptr);
